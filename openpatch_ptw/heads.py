...
class LocalCodeHead(nn.Module):
    """Predict the local position code from GenPTW's shared watermark feature."""
...
class OpenSetStatusHead(nn.Module):
    ...
    def __init__(self, wm_channels: int = 1, hidden_dim: int = 128, num_classes: int = 3):
        ...
        self.net = nn.Sequential(
            nn.Linear(wm_channels + 4, hidden_dim),
    ...
    def forward(
        ...
        pooled = self.pool(wm_feature[:, :1]).flatten(1)
        bit_confidence = (decoded_bits - 0.5).abs().mul(2.0)
        bit_mean = bit_confidence.mean(dim=1, keepdim=True)
        bit_min = bit_confidence.amin(dim=1, keepdim=True)
        residual_mean = residual_map.mean(dim=(1, 2, 3), keepdim=False).unsqueeze(1)
        residual_max = residual_map.amax(dim=(1, 2, 3), keepdim=False).unsqueeze(1)
        return self.net(torch.cat([pooled, bit_mean, bit_min, residual_mean, residual_max], dim=1))
